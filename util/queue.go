...
var instanceTypePool = NewSingleInstanceType[*ants.Pool](func() *ants.Pool {
	pool, _ := ants.NewPool(1000, ants.WithPreAlloc(true))
	return pool
})

var GoPool = instanceTypePool.Load()

//var GoPool, _ = ants.NewPool(1000, ants.WithPreAlloc(true))

type GQueue struct {
	mu       sync.RWMutex
	opt      GQueueOption
	qu       *linkedlistqueue.Queue
