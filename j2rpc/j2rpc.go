...
func (s *server) Handler(ctx *Context) {
	...
	ctx.Msg().output().writeResponse(w, s.Opt().CryptoKey)
...
func (s *server) handle(c *Context) (err error) {
	...
	if cryptoStr := s.Opt().CryptoKey; len(cryptoStr) >= 16 {
		func() {
			defer func() {
				if e := recover(); e != nil {
					err = NewError(400, "参数解析错误")
					return
				}
			}()
			body, err = goutil.AESCBCDecrypt([]byte(cryptoStr), bytes.ToLower(body))
			if err != nil {
				err = NewError(400, "参数解析错误")
				return
			}
