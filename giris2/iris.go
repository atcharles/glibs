...
import (
	...
	rev "github.com/kataras/iris/v12/middleware/recover"
...
func (a *AppStart) defaultMiddlewares(p iris.Party) {
	...
	p.UseRouter(
		...
		cors.New().ExposeHeaders("X-Server", "Authorization", "X-Authorization", "Request-Id", "X-Request-Id").
			AllowHeaders("Authorization", "X-Authorization", "Request-Id", "X-Request-Id",
				"X-Server", "token", "Accept", "Accept-Language", "Content-Language", "Content-Type").
			Handler(),
	)
}
