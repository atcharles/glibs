...
var supportMethods = []string{"GET", "POST", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS", "CONNECT", "TRACE"}
...
// func Handler(c *lhttp.Context,bound any) (any, error)
// func Handler(c *lhttp.Context,bound any) any
// func Handler(c *lhttp.Context,bound any) error
// func Handler(c *lhttp.Context,bound any)
...
func parseRestFuncResult(result []reflect.Value, c iris.Context) error {
	...
	if last.Type().Implements(errorType) {
		if !last.IsZero() {
			var ee *j2rpc.Error
			switch {
			case errors.As(last.Interface().(error), &ee):
				return c.JSON(JSONResponse{Code: int(ee.Code), Msg: ee.Message})
			default:
				return c.JSON(JSONResponse{Code: 500, Msg: last.Interface().(error).Error()})
			}
		}
