...
func (g *GormDB) GetModel(table string) (interface{}, error) {
	...
	return nil, errors.New("table's model not found")
}
...
func (g *GormDB) RegModelBus(bus interface{}) {
	models := j2rpc.ObjectTagInstances(bus, "model")
...
func (g *GormDB) MigrateModels(v ...interface{}) {
	models := append(v, globalDBModels...)
	...
	if config.Viper().GetInt("app.runtimes") > 1 {
		return
	}
...
func (d *DBOption) parseDSN() string {
	...
	case "mysql":
		...
		dsn = TextTemplateMustParse(dsn, d)
		return dsn
	case "pg", "postgres":
		// sslmode=disable
		dsn := `host={{.host}}
...
func (d *DBOption) GetLogger() logger.Interface {
	if d.Logger == nil {
		d.Logger = NewDBLoggerWithLevel(logger.LogLevel(config.Viper().GetInt("db.log_level")))
	}
	return d.Logger
}
...
var defaultDatetimePrecision = 9
...
func (d *DBOption) dbInitiate() (db *gorm.DB, err error) {
	...
	case "pg", "postgres":
		dialectal = postgres.New(postgres.Config{
			DriverName:           "pgx",
			DSN:                  dsn,
...
func (*argsTagModel) deleteByDays(db *gorm.DB, model interface{}, days int) {
	...
	days--
	str := carbon.FromStdTime(time.Now()).AddDays(-days).ToDateString()
	time1 := carbon.Parse(str).ToStdTime()
	var count int64
